...
function Bsl(device, cmd, message, receive_response) {
  let txdata = [0x3f, 1 + message.length, cmd].concat(message);
  let padding_size = 64 - txdata.length;
  ...
  if (receive_response) {
    let report = device.readSync();
    let pi = report[0];

    if ((pi = 0x3f)) {
      let length = report[1];
      let data = report.slice(2, 2 + length);
...
function BslRxPassword(device, password) {
  let answer = Bsl(device, Commands.BSL_RX_PASSWORD, password, true);
  CheckAnswer(answer);
}

// open a device type mps430 in bsl mode
let device = new HID.HID(0x2047, 0x0200);

// only for test readTimeout function
let data_read = device.readTimeout(1000);
console.log('data_read: ' + JSON.stringify(data_read));

// init for mass erase password
let package_erase_password = [];
